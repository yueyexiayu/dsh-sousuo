...
import { describeNetworkError, fetchWithFailover } from "./transport.js";
...
export class AnySearchClient {
    ...
    async requestOnce(url, operation, init, signal, apiKey, authentication) {
        ...
        let response;
        try {
            response = await fetchWithFailover(url, {
                method: init.method,
                redirect: 'error',
                headers,
                ...init.body === undefined ? {} : { body: init.body },
                signal: requestSignal,
            }, this.options.transportHooks);
        }
        catch (error) {
            ...
            if (isAbortError(error))
                throw aborted(operation, signal, error);
            throw new AnySearchClientError(`AnySearch ${operation} request failed: ${describeNetworkError(error)}`, { operation, cause: error });
        }
